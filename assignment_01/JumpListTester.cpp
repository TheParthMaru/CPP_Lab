...
void JumpListTester::testc()
{
    funcname_ = "JumpListTester::testc";
    {
        ...
        JumpList a;
        if (a.find("qqq"))
            errorOut_("found something in empty list", 1);

        JumpList b(3);
        if (!b.find("a"))
            errorOut_("did not find a when present", 1);
        if (!b.find("b"))
            errorOut_("did not find b when present", 1);
        if (!b.find("blah"))
            errorOut_("did not find blah when present", 1);
        if (b.find("0"))
            errorOut_("found 0 when absent", 1);
        if (b.find("aaa"))
            errorOut_("found aaa when absent", 1);
        if (b.find("zzz"))
            errorOut_("found zzz when absent", 1);

        JumpList c(10);
        if (!c.find("cat"))
            errorOut_("did not find cat when present", 2);
        if (!c.find("d"))
            errorOut_("did not find d when present", 2);
        if (!c.find("ooo..."))
            errorOut_("did not find ooo... when present", 2);
        if (!c.find("x"))
            errorOut_("did not find x when present", 2);
        if (!c.find("zzz"))
            errorOut_("did not find zzz when present", 2);
        if (c.find("dog"))
            errorOut_("found dog when absent", 2);
        if (c.find("zzzz"))
            errorOut_("found zzzz when absent", 2);
    }
    passOut_();
}
...
void JumpListTester::teste()
{
    funcname_ = "JumpListTester::teste";
    {
        ...
        s = a.get(-1);
        if (s != "")
            errorOut_("get out of bounds -1 wrong: ", s, 1);
        s = a.get(10);
        if (s != "")
            errorOut_("get out of bounds 10 wrong: ", s, 1);

        JumpList b;
        s = b.get(0);
        if (s != "")
            errorOut_("get out of bounds wrong for empty jumplist: ", s, 2);
    }
    passOut_();
}
